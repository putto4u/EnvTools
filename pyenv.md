...
---
## 🐍 Ubuntu에서 Python 3.11.6 설치 가이드

Ubuntu 환경에서 \*\*`pyenv`\*\*를 사용하여 Python 3.11.6 버전을 설치하는 가장 안정적이고 권장되는 방법을 안내해 드립니다. 이 방법은 시스템 Python 환경을 오염시키지 않고, 향후 프로젝트별 버전 관리를 용이하게 합니다.

-----

### 1\. 🛠️ 사전 준비: pyenv 및 빌드 도구 확인

Python 소스 코드를 컴파일하여 설치하려면, `pyenv`가 설치되어 있어야 하며 필요한 빌드 도구가 시스템에 갖춰져 있어야 합니다.
...
`pyenv`가 정상적으로 설치 및 초기화되었는지 확인합니다.

```bash
pyenv --version
```

만약 오류가 발생하거나 버전이 출력되지 않으면, 이전 단계에서 진행한 \*\*pyenv 설치 및 쉘 환경 설정(`source ~/.bashrc`)\*\*을 다시 확인해야 합니다.

-----

### 2\. 📥 Python 3.11.6 설치

`pyenv install` 명령어를 사용하여 원하는 Python 버전을 다운로드하고 컴파일합니다.

```bash
pyenv install 3.11.6
```

> **참고:** 이 과정은 소스 코드를 다운로드하고 컴파일하기 때문에, 시스템 성능에 따라 \*\*상당한 시간(5\~10분 이상)\*\*이 소요될 수 있습니다.

-----

### 3\. ✅ 설치 완료 및 버전 확인

설치가 완료되면, 설치된 버전 목록에 `3.11.6`이 나타나는지 확인합니다.
...
```
  system
  3.10.12
  3.11.6  <-- 여기에 방금 설치한 버전이 나타나야 합니다.
```

-----

### 4\. ⚙️ Python 3.11.6 활성화 (Global 또는 Local)

설치된 버전을 실제로 사용하려면 `pyenv`에 해당 버전을 활성화하도록 지정해야 합니다.

#### A. Global 설정 (전역 기본 버전)

**시스템 전체**의 기본 Python 버전을 `3.11.6`으로 설정합니다. (다만, 시스템 환경을 보호하기 위해 **권장하지는 않습니다**.)

```bash
pyenv global 3.11.6
```

#### B. Local 설정 (프로젝트별 격리 - 권장)

**3vm구축실습예제**와 같이 특정 프로젝트 폴더에서만 `3.11.6`을 사용하도록 설정합니다. (가장 모범적인 방법)

```bash
# 1. 프로젝트 폴더 생성 및 이동
mkdir my_project_311
cd my_project_311

# 2. 이 폴더에서만 3.11.6 사용 지정
pyenv local 3.11.6
